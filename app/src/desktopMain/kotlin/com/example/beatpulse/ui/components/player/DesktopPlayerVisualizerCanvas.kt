...
fun DesktopPlayerVisualizerCanvas(
    ...
    Canvas(modifier = modifier.size(320.dp).graphicsLayer {
        ...
        val pathLength = if (thumbnailShapeIdx == 0) (rPx * 2f * kotlin.math.PI).toFloat() else progressMeasure.length
        ...
        fun drawLayer(amps: FloatArray, layerColor: Color, opacity: Float, isBassLayer: Boolean) {
            ...
            fun computePointAndNormal(d: Float) {
                if (thumbnailShapeIdx == 0) {
                    val angle = (d / pathLength) * 2 * kotlin.math.PI - kotlin.math.PI / 2
                    ...
                    if (pos != Offset.Unspecified && tan != Offset.Unspecified) {
                ...
                VisualizerStyle.SLIME -> {
                    ...
                    if (totalPoints > 0) {
                        ...
                        for (i in 0 until totalPoints) {
                            ...
                            wavePathR.quadraticBezierTo(slimeX[i], slimeY[i], midX, midY)
                        }
                        wavePathR.close()
